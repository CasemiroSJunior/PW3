...
class ListarFeriadosViewTest(TestCase):
    ...
    def test_listar_feriados_vazio(self):
        response = self.client.get(reverse('listar_feriados')) ## reverse procura pelo 'name' no pela url
...
class AdicionarFeriadoViewTest(TestCase):
    ...
    def test_get_formulario_adicionar_feriado(self):
        response = self.client.get(reverse('adicionar_feriado'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_post_dados_validos_cria_feriado(self):
        ...
        response = self.client.post(reverse('adicionar_feriado'), data)
    ...
    def test_post_faltando_dados_retorna_200(self):
        response = self.client.post(reverse('adicionar_feriado'), {'nome': '', 'dia': '', 'mes': ''})
        self.assertEqual(response.status_code, 200)

    def test_post_dia_ou_mes_fora_do_intervalo(self):
        response = self.client.post(reverse('adicionar_feriado'), {'nome': 'Feriado', 'dia': '32', 'mes': '13'})
        self.assertEqual(response.status_code, 200)

    def test_post_dia_mes_nao_numerico(self):
        response = self.client.post(reverse('adicionar_feriado'), {'nome': 'Teste', 'dia': 'dez', 'mes': 'jan'})
        self.assertEqual(response.status_code, 200)